$(document).ready(function(){
    ...
    var knot_tile = {
        ...
        Height: 1.3,
        Pnts: [
            [1, 1], [2, 0], [4, 0], [5, 1], [3, 1], [2, 0], [0, 0], [1, 1], [3, 1], [4, 0], [5, 0], [6, 1],
        ]
    };
    ...
    // apply_template(knot_tile, [0, 0], 50, 100, 7);

    // we may not reach outer_rad, but that's where a next layer would go...
    function apply_template(tile, centre, inner_rad, outer_rad, repeats, circles) {
        var points = [];
        ...
        function transform_point(pnt, base_ang_idx) {
            ...
            var rad_mod = ang_idx / num_angles;

            if (ang_idx > num_angles) {
                ang_idx -= num_angles;
            }

            var rad = inner_rad + pnt[1] * rad_step + (outer_rad - inner_rad) * rad_mod;

            return [ centre[0] + rad * Math.sin(ang_idx * ang_step),
                     centre[1] + rad * -Math.cos(ang_idx * ang_step) ];
        }

        for(var base_ang_idx = 0; base_ang_idx < num_angles * circles; base_ang_idx += tile.Width )
        {
            tile.Pnts.forEach(pnt => {
                points.push(transform_point(pnt, base_ang_idx));
            });
    ...
    apply_template(knot_tile, [0, 0], 50, 75, 7, 3);
});